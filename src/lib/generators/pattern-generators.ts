...
function generateAddSubDoubleMixed(_settings: WorksheetSettings, count: number): BasicProblem[] {
  const problems: BasicProblem[] = [];
  const usedCombinations = new Set<string>();

  for (let i = 0; i < count; i++) {
    let operand1: number = 0, operand2: number = 0;
    ...
    problems.push({
      ...
      operand1,
      operand2,
      answer: operation === 'addition' ? operand1 + operand2 : operand1 - operand2,
      carryOver,
    });
  }
