...
- **図やダイアグラムに答えをそのまま表示する**: 教育用の図（Bar Model、Number Bondなど）を問題に組み込む際、求める値は `?` や空欄にし、生徒が計算して求める余地を残すこと。型定義に `hidden` フィールドを設けて隠す値を構造的に管理する。実装後はPlaywright MCPのスクリーンショットで「問題として成立しているか」を必ず目視確認する
